...
# Cleaning Process Summar

1. Removed rows where the target variable FAH is missing
2. Converted FAH and other categorical variables to numeric
3. Extracted Year and Month from the Date column
4. Removed unnecessary columns from the dataset: OSgrid, OAH, Obs, Location, longitude, latitude, Date
5. Removed impossible & very extreme values
6. Transformed the circular variables into sine and cosine features so that they can be meaningful in the neural network
7. Investigated problematic variables...Still need to figure out what to do with these
...
data <- read.csv("C:/Users/Student/Documents/DS4I_ASSIGNMENT_1/scotland_avalanche_forecasts_2009_2025.csv", header = TRUE)
...
* "It is worthy of note that the highest wind speed recorded at the summit weather station on Cairngorm at the time of writing was an eye-watering 176 mph!"  https://www.mountaineering.scot/safety-and-skills/essential-skills/weather-conditions/wind-speeds
...
*VERY HELPUL DOC FROM THE SAME SITE THE DATA WAS COLLECTED FROM:*
...
data_cleaned <- data %>%
  mutate(
    ...
    # Summit Wind Speed (mph) (same threshold as Wind Speed): removed quite a few values
    ...
    # Snow Temperature
    # Snow melts > 0 degrees Celsius
    # 0 to 0.5 degrees Celsius is a plausible range for melting snow
...
### Investigating Max.Temp.Grad (centigrade over 10 centimetres throughout the snowpack depth)

* 4635 rows of values = 0
* 642 NA.
* Must do more research here

"Generally if the temperature gradient is around 1°C/10 cms or more (colder nearer the surface) this may indicate weakness may develop or are present in the snowpack..."

ChatGPT: High gradient threshold:
* ≥ 1 °C per 10 cm is considered high.
* This means if the temperature changes by 1°C or more over 10 cm of snow depth, the gradient is strong enough to drive faceted crystal growth (weak layer formation).


Evidence of a gradient of 11 (Greenland ice sheet snowpack): https://www.researchgate.net/publication/300001311_Observaciones_del_manto_de_nieve_durante_una_circunnavegacion_del_casquete_de_hielo_de_Groenlandia_primavera_de_2014/figures?lo=1


```{r}
summary(data$Max.Temp.Grad, na.rm = TRUE)
na.omit(data$Max.Temp.Grad)
plot(data$Max.Temp.Grad, data$FAH) # a good visualization

data[data$Max.Temp.Grad > 3 & !is.na(data$Max.Temp.Grad), ]
```
...
It actually seems like its almost a categorical variable? Which is nottttt helpful based on the number of groups, etc...

```{r}
summary(data$No.Settle)
table(data$No.Settle) # makes no sense
table(data$No.Settle, data$FAH) # makes no sense

# Not seeing anything that shows any sign that this features actually means something....
plot(data$No.Settle, data$Total.Snow.Depth)
data %>% select(Foot.Pen, No.Settle, Total.Snow.Depth) %>% na.omit()

```

### Investigating Ski Pen variable - REMOVE
...
### Investigating Crystals & Wetness - REMOVE

* How to deal with these variables?
* -1 must mean no result/na?
* The categories aren't making sense

According to the SAIF this is just not how these things are measured....
...
### Investigating AV.Cat - REMOVE

100% must be left out for so many reasons. The SAIC uses Avalanche Code not Avalanche Category  
...
### Investigating Insolation - REMOVE
...
### Investigating Snow.Index - REMOVE
...
# Getting rid of bad variables

* Ski.Pen - 2,404 out of 10,671 records (22.5%) missing - too much missing data for reliable neural network training
* AV.Cat - 2,494 records (23.4%) missing & manyyy values that are DEFINITLY errors

* We excluded features "crystals", "wetness", "snow index", "Insolation" as they did not align with the The International Classification for Seasonal Snow on the Ground (reference here). These feature measures also made no sense supporting the reference doc.
* I'm keeping month in the dataset because it captures seasonality, however year is actually not useful cause it can act as a proxy for changes such as  improved reporting after 2015....
* I vote No.Settle is removed. Not finding anything about it in that International Classification doc that suggests the measures we have make any sense at all. 

I'm having trouble figuring out what to do with Max.Temp.Grad. There are a LOT of outliers but not sure where to make the cutoff point
