...
From the table it is evident that predictor sets 2 and 4 perform relatively equally with validation accuracies of 66% and 65% respectively. Predictor set 3 has slightly worse predictive capability with a prediction accuracy around 63%. Predictor set 1 is firmly the worst, achieving 50% validation accuracy. It is clear that the variables relating to the location are not as important as the variables relating to the weather conditions and the snow pack test. The rest of the predictor sets perform roughly the same and the results are close enough for the difference to be down to randomness. It is expected that predictor set 4(containing all variables) would perform at least as good as the best of the first three predictor sets since the best predictor set would be contained in predictor set 4. The best model configuration when using predictor set 4 is summarised below.
...
The best configuration for predictor set 4 has 4 hidden layers, with sizes 30, 40, 40 and 40 nodes respectively. This model achieved a validation accuracy of 64.9% in training. The hidden layers all have Relu activation functions and the output layer uses a softmax activation. Softmax is used because we have multiple categories in our traget variable.
