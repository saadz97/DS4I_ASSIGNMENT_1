...
con_mat =  tidyr::pivot_wider(con_mat, names_from = Reference, values_from = Freq,
                              values_fill = 0)

#con_mat = mutate(con_mat, Row_Label = "Predicted")
kable(con_mat, format = 'html', caption = 'Confusion matrix') %>%
  kable_styling(full_width = FALSE, position = 'center') %>%
  add_header_above(c(" " = 1, "Reference" = 5))

kable(con_mat$byClass, format = 'html', caption = 'Class metrics', digits = 3) %>%
  kable_styling(full_width = FALSE, position = 'center')
